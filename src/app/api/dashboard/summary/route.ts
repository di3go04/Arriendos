...
export async function GET(req: Request) {
  try {
    ...
    // Demo mode: allow access without auth, use a fallback user ID
    ...
    const [leadsData, contractsData, paymentsData, propsData] = await Promise.all([
      supabase.from('property_leads').select('*').eq('owner_id', userId).order('created_at', { ascending: false }).limit(5),
      supabase.from('contracts').select('id, monthly_rent, status, tenant_id, property:properties(title), tenant:profiles!contracts_tenant_id_fkey(full_name, email)').eq('landlord_id', userId),
      supabase.from('payments').select('amount, paid, due_date, contract_id, tenant_id').eq('tenant_id', tenantId || userId),
      supabase.from('properties').select('id, title, monthly_rent, status').eq('owner_id', userId),
    ]);
