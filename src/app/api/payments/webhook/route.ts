...
export async function POST(req: Request) {
  const { valid, body: parsed, response } = await verifyWebhookRequest(req, MP_WEBHOOK_SECRET, {
  ...
  try {
    ...
    const paymentData = await payment.get({ id: String(paymentId) }) as unknown as LooseRecord;
