...
function getStatusConfig(tc: ReturnType<typeof useTranslations>): Record<string, { label: string; cls: string }> {
  return {
    borrador: { label: tc('status_draft'), cls: 'bg-muted border-border text-muted-foreground' },
    pendiente_firma: { label: tc('status_pending'), cls: 'bg-amber-500/10 border-amber-500/25 text-amber-500' },
    firmado: { label: tc('status_signed'), cls: 'bg-blue-500/10 border-blue-500/25 text-blue-500' },
    activo: { label: tc('status_active'), cls: 'bg-slate-100 border-slate-200 text-slate-700' },
    finalizado: { label: tc('status_completed'), cls: 'bg-slate-500/10 border-slate-500/25 text-slate-500' },
    cancelado: { label: tc('status_cancelled'), cls: 'bg-red-50 border-red-200 text-red-600' },
  };
}
...
export default function ContractDetailPage() {
  ...
  const t = useTranslations('contract_detail');
  ...
  const handleAction = async (action: 'finalizar' | 'cancelar' | 'renovar') => {
    ...
    try {
      ...
      if (action === 'renovar') {
        ...
        setSuccessMsg(t('renovate_success'));
      ...
      setSuccessMsg(t('action_success', { action: action === 'finalizar' ? 'finalizado' : 'cancelado' }));
      fetchData();
  ...
  // Timeline
  const buildTimeline = (t: ReturnType<typeof useTranslations>, tc: ReturnType<typeof useTranslations>) => {
    if (!contract) return [];
    ...
    events.push({
      ...
      type: 'created',
      title: t('timeline_created'),
      desc: t('timeline_created_desc', { number: contract.contract_number || contract.id.slice(0, 8) }),
      icon: FileText,
    ...
    if (contract.signed_by_landlord && contract.landlord_signed_at) {
      events.push({
        ...
        title: t('timeline_landlord_signed'),
        desc: t('timeline_landlord_signed_desc', { name: contract.landlord?.full_name || tc('landlord') }),
        icon: FileSignature,
    ...
    if (contract.signed_by_tenant && contract.tenant_signed_at) {
      events.push({
        ...
        type: 'tenant_signed',
        title: t('timeline_tenant_signed'),
        desc: t('timeline_tenant_signed_desc', { name: contract.tenant?.full_name || tc('tenant') }),
        icon: CheckCircle2,
        cls: 'text-blue-600 bg-blue-50 border-blue-200',
      });
    }
    ...
    if (contract.status === 'activo') {
      events.push({
        ...
        type: 'activated',
        title: t('timeline_activated'),
        desc: t('timeline_activated_desc'),
        icon: ShieldCheck,
        cls: 'text-blue-600 bg-blue-50 border-blue-200',
      });
    }
    if (contract.status === 'finalizado') {
      events.push({
        ...
        type: 'finalized',
        title: t('timeline_finalized'),
        desc: t('timeline_finalized_desc'),
        icon: Ban,
    ...
    if (contract.status === 'cancelado') {
      events.push({
        date: new Date().toISOString(),
        type: 'cancelled',
        title: t('timeline_cancelled'),
        desc: t('timeline_cancelled_desc'),
        icon: XCircle,
    ...
    for (const p of payments) {
      events.push({
        ...
        type: 'payment',
        title: p.paid ? t('timeline_payment_made') : t('timeline_payment_scheduled'),
        desc: `${formatCOP(p.amount)} — ${p.paid ? t('paid') : t('pending')}${p.payment_method ? ` (${p.payment_method})` : ''}`,
    ...
    for (const d of documents) {
      events.push({
        date: d.created_at,
        type: 'document',
        title: t('timeline_document_uploaded', { name: d.name || t('document') }),
        desc: t('timeline_document_desc', { type: d.type, uploader: d.uploader?.full_name || t('unknown') }),
        icon: FileText,
        cls: 'text-blue-500 bg-blue-500/10 border-blue-500/20',
      });
    }
  ...
  if (accessDenied || !contract) {
    return (
      <div className="min-h-[80vh] flex flex-col items-center justify-center gap-3">
        ...
        <button onClick={() => router.push('/dashboard/leases')} className="text-xs text-primary font-bold hover:underline cursor-pointer">{t('back')}</button>
      </div>
    );
  }

  const timeline = buildTimeline(t, tc);
  const statusConfig = getStatusConfig(tc);
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 pb-16 animate-fade-in">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-extrabold text-foreground flex items-center gap-2">
              {tc('contract_number', { number: contract.contract_number || contract.id.slice(0, 8) })}
      ...
      <div className="flex items-center gap-1 border-b border-border overflow-x-auto">
        {(['resumen', 'pagos', 'documentos', 'historial'] as const).map(tabKey => (
          <button
            key={tabKey}
          ...
          >
            {tabKey === 'resumen' && <FileText className="w-4 h-4" />}
            {tabKey === 'pagos' && <DollarSign className="w-4 h-4" />}
            {tabKey === 'documentos' && <FileText className="w-4 h-4" />}
            {tabKey === 'historial' && <Clock className="w-4 h-4" />}
            {tabKey === 'resumen' && t('tab_summary')}
            {tabKey === 'pagos' && t('tab_payments')}
            {tabKey === 'documentos' && t('tab_documents')}
            {tabKey === 'historial' && t('tab_timeline')}
          </button>
        ))}
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-card border border-border rounded-3xl p-6 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                <InfoItem icon={FileText} label={t('number')} value={`#${contract.contract_number || contract.id.slice(0, 8)}`} />
                <InfoItem icon={Building2} label={t('property')} value={
                ...
                <InfoItem icon={MapPin} label={t('address')} value={`${contract.property?.address || ''}${contract.property?.city ? `, ${contract.property.city}` : ''}`} />
                <InfoItem icon={User} label={tc('landlord')} value={
                  <span>
                ...
                <InfoItem icon={User} label={tc('tenant')} value={
                  <span>
                ...
                <InfoItem icon={DollarSign} label={tc('monthly_rent')} value={formatCOP(contract.monthly_rent)} />
                <InfoItem icon={DollarSign} label={tc('deposit')} value={formatCOP(contract.deposit)} />
                <InfoItem icon={Calendar} label={tc('start_date')} value={contract.start_date ? format(parseISO(contract.start_date), 'dd/MMM/yyyy', { locale: es }) : '—'} />
                <InfoItem icon={Calendar} label={tc('end_date')} value={contract.end_date ? format(parseISO(contract.end_date), 'dd/MMM/yyyy', { locale: es }) : tc('indefinite')} />
                <InfoItem icon={Calendar} label={t('payment_day')} value={t('payment_day_value', { day: contract.payment_day })} />
            </div>
          </div>
              ...
              <div className="p-4 rounded-2xl border border-border/60">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-sm text-foreground">{tc('landlord')}</span>
                  </div>
                  {contract.signed_by_landlord ? (
                    <span className="inline-flex items-center gap-1 bg-blue-50 border border-blue-200 text-blue-600 text-[10px] font-bold px-2 py-0.5 rounded-full">
                      <CheckCircle2 className="w-3 h-3" /> {t('signed')}
                    </span>
                  ) : (
              ...
              <div className="p-4 rounded-2xl border border-border/60">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-sm text-foreground">{tc('tenant')}</span>
                  </div>
            ...
            <div className={`rounded-3xl p-6 shadow-sm border ${
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="block text-2xl font-extrabold text-foreground">{remainingDays > 0 ? t('days_remaining', { count: remainingDays }) : t('expired')}</span>
                    <span className="block text-[11px] font-bold text-muted-foreground">
                      {remainingDays > 0 ? t('days_remaining_label') : t('contract_expired_label')}
                    </span>
                  </div>
                </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4">
              <span className="block text-[10px] font-bold text-muted-foreground uppercase tracking-wider">{t('total_paid')}</span>
              <span className="block text-xl font-extrabold text-foreground mt-1">{formatCOP(totalPaid)}</span>
            </div>
            <div className="bg-amber-500/5 border border-amber-500/20 rounded-2xl p-4">
              <span className="block text-[10px] font-bold text-muted-foreground uppercase tracking-wider">{t('total_pending')}</span>
              <span className="block text-xl font-extrabold text-foreground mt-1">{formatCOP(totalPending)}</span>
            </div>
            <div className="bg-blue-500/5 border border-blue-500/20 rounded-2xl p-4">
              <span className="block text-[10px] font-bold text-muted-foreground uppercase tracking-wider">{t('next_payment')}</span>
              <span className="block text-xl font-extrabold text-foreground mt-1">
                {nextPayment ? formatCOP(nextPayment.amount) : '—'}
              </span>
              {nextPayment && (
                <span className="block text-[10px] text-muted-foreground mt-0.5">
                  {t('due_date_label')} {format(parseISO(nextPayment.due_date), 'dd/MMM/yyyy', { locale: es })}
                </span>
              )}
            </div>
          </div>
          ...
          {payments.length === 0 ? (
            <div className="py-10 text-center bg-card border border-dashed border-border rounded-3xl">
              ...
              <p className="text-xs font-bold text-muted-foreground">{t('no_payments')}</p>
            </div>
          ) : (
                ...
                <table className="w-full text-xs text-left border-collapse">
                  <thead>
                    <tr className="bg-muted/40 border-b border-border text-muted-foreground font-bold text-[10px] uppercase tracking-wider">
                      <th className="px-5 py-4">{t('table_month')}</th>
                      <th className="px-5 py-4">{t('table_amount')}</th>
                      <th className="px-5 py-4">{t('table_due_date')}</th>
                      <th className="px-5 py-4">{t('table_status')}</th>
                      <th className="px-5 py-4">{t('table_paid_date')}</th>
                      <th className="px-5 py-4">{t('table_method')}</th>
                      <th className="px-5 py-4">{t('table_receipt')}</th>
                    </tr>
                  </thead>
        ...
        <div className="space-y-4">
          ...
          {documents.length === 0 ? (
            <div className="py-10 text-center bg-card border border-dashed border-border rounded-3xl">
              ...
              <p className="text-xs font-bold text-muted-foreground">{t('no_documents')}</p>
            </div>
          ) : (
              ...
              {documents.map(d => (
                <div key={d.id} className="bg-card border border-border rounded-2xl p-5 shadow-sm hover:shadow-md transition-all">
                  <span className={`inline-flex items-center gap-1.5 text-[10px] font-bold px-2.5 py-1 rounded-full border mb-3 ${
                    ...
                    {d.type === 'inventario' ? t('inventory') : d.type === 'foto' ? t('photo') : d.type === 'anexo' ? t('annex') : t('other')}
                  </span>
                  <h4 className="font-bold text-sm text-foreground mb-2 break-words">{d.name || t('document')}</h4>
        ...
        <div>
          {timeline.length === 0 ? (
            <div className="py-10 text-center bg-card border border-dashed border-border rounded-3xl">
              ...
              <p className="text-xs font-bold text-muted-foreground">{t('no_events')}</p>
            </div>
          ) : (
