...
export default function NewContractWizard() {
  ...
  const t = useTranslations('contract_wizard');
  ...
  const handleNext = () => {
    if (state.step === 3) {
      ...
      else set({ errorMsg: t('error_compile') });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      set({ errorMsg: err instanceof Error ? err.message : t('error_create'), isSubmitting: false });
    }
  };
    ...
    <div className="min-h-screen bg-[#f8fafc]">
      <div className="flex">
        ...
        <main className="flex-1 px-4 md:px-8 pt-6 pb-24 max-w-5xl mx-auto">
          <BackToHome href="/dashboard/landlord">{t('back_to_dashboard')}</BackToHome>
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">{t('step_of', { step: state.step, total: 5 })}</p>
            </div>
            <div className="hidden md:flex items-center gap-1">
              {STEPS.map(s => (
                <div key={s.num} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-bold transition-colors ${
                  ...
                  <span className="hidden lg:inline">{s.num === 1 ? t('step_property') : s.num === 2 ? t('step_tenant') : s.num === 3 ? t('step_template') : s.num === 4 ? t('step_review') : t('step_signature')}</span>
                </div>
              ))}
            </div>
          </div>
                ...
                <>
                  {state.properties.length === 0 ? (
                    <div className="text-center py-16">
                      ...
                      <p className="font-bold text-foreground mb-1">{t('no_properties')}</p>
                      <p className="text-xs text-muted-foreground mb-4">{t('no_properties_desc')}</p>
                      <a href="/properties" className="inline-flex items-center gap-1.5 text-sm font-bold text-white bg-primary px-5 py-2.5 rounded-xl">{t('create_property')}</a>
                    </div>
                  ) : (
                        ...
                        return (
                          <button key={p.id} onClick={() => set({ selectedProperty: p })}
                            ...
                            <div className="flex items-center gap-4 text-xs">
                              <span className="font-bold text-foreground tabular-nums">{formatCOP(p.monthly_rent)}{t('per_month')}</span>
          ...
          {state.step === 2 && (
            <div className="space-y-4">
              <div className="flex items-center gap-2 border-b border-border pb-4">
                {(['search', 'create', 'manual'] as const).map(mode => (
                  <button key={mode} onClick={() => set({ tenantMode: mode })}
                    ...
                    {mode === 'search' ? t('search') : mode === 'create' ? t('create_new') : t('manual')}
                  </button>
                ))}
              </div>
                ...
                <div className="space-y-3">
                  <div className="relative">
                    ...
                    <input type="text" value={state.tenantSearchQuery} onChange={e => set({ tenantSearchQuery: e.target.value })}
                      placeholder={t('search_placeholder')} className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-border bg-card text-sm outline-none focus:border-primary" />
                  </div>
                  {state.searchingTenant && <Loader2 className="w-4 h-4 animate-spin text-primary mx-auto" />}
                   {state.tenantResults.map(tenant => (
                     <button key={tenant.id} onClick={() => set({ selectedTenant: tenant })}
                       className={`w-full text-left p-4 rounded-xl border-2 transition-all cursor-pointer ${
                         state.selectedTenant?.id === tenant.id ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/30'
                       }`}>
                       <div className="flex items-center gap-3">
                         ...
                         <div>
                           <p className="text-sm font-bold text-foreground">{tenant.full_name}</p>
                           <p className="text-[11px] text-muted-foreground">{tenant.email || t('no_email')}</p>
                         </div>
                         {state.selectedTenant?.id === tenant.id && <Check className="w-5 h-5 text-primary ml-auto" />}
                       </div>
                     </button>
                   ))}
                </div>
              ...
              {(state.tenantMode === 'create' || state.tenantMode === 'manual') && (
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs font-semibold text-muted-foreground mb-1 block">{t('full_name')}</label>
                    <input type="text" value={state.tenantMode === 'create' ? state.createTenantData.full_name : state.manualTenantData.full_name}
                      ...
                      placeholder={t('tenant_name_placeholder')} className="w-full px-3 py-2.5 rounded-xl border border-border bg-card text-sm outline-none focus:border-primary" />
                  </div>
                  <div>
                    ...
                    <input type="email" value={state.tenantMode === 'create' ? state.createTenantData.email : state.manualTenantData.email}
                      ...
                      placeholder={t('email_placeholder')} className="w-full px-3 py-2.5 rounded-xl border border-border bg-card text-sm outline-none focus:border-primary" />
                  </div>
                  {state.tenantMode === 'create' && (
                    <div>
                      <label className="text-xs font-semibold text-muted-foreground mb-1 block">{t('phone')}</label>
                      <input type="tel" value={state.createTenantData.phone} onChange={e => set({ createTenantData: { ...state.createTenantData, phone: e.target.value } })}
                        placeholder={t('phone_placeholder')} className="w-full px-3 py-2.5 rounded-xl border border-border bg-card text-sm outline-none focus:border-primary" />
                    </div>
                  )}
                  {state.tenantMode === 'manual' && (
                    <div>
                      <label className="text-xs font-semibold text-muted-foreground mb-1 block">{t('doc_id')}</label>
                      <input type="text" value={state.manualTenantData.doc_id} onChange={e => set({ manualTenantData: { ...state.manualTenantData, doc_id: e.target.value } })}
                        placeholder={t('doc_id_placeholder')} className="w-full px-3 py-2.5 rounded-xl border border-border bg-card text-sm outline-none focus:border-primary" />
                    </div>
                  )}
                </div>
              )}
              ...
              {state.loadingDeps ? (
                ...
                <>
                  {state.templates.length === 0 ? (
                    <div className="text-center py-16"><p className="font-bold text-foreground">{t('no_templates')}</p></div>
                  ) : (
                    <div className="grid sm:grid-cols-2 gap-4">
                      {state.templates.map(template => (
                        <button key={template.id} onClick={() => set({ selectedTemplate: template })}
                          className={`text-left p-5 rounded-2xl border-2 transition-all cursor-pointer ${
                            state.selectedTemplate?.id === template.id ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/30'
                          }`}>
                          <div className="flex items-center gap-3 mb-2">
                            <div className={`p-2 rounded-xl ${state.selectedTemplate?.id === template.id ? 'bg-primary text-white' : 'bg-muted text-muted-foreground'}`}>
                              <FileText className="w-5 h-5" />
                            </div>
                            <div>
                              <p className="text-sm font-bold text-foreground">{template.name}</p>
                            </div>
                            {state.selectedTemplate?.id === template.id && <Check className="w-5 h-5 text-primary ml-auto" />}
                          </div>
                          {template.is_public && <span className="text-[10px] font-bold text-primary bg-primary/10 px-2 py-0.5 rounded-full">{t('public')}</span>}
                        </button>
                      ))}
                    </div>
                  )}
                  ...
                  <div className="grid sm:grid-cols-3 gap-4 mt-4">
                    <div>
                      <label className="text-xs font-semibold text-muted-foreground mb-1 block">{t('start_date')}</label>
                    ...
                    <div>
                      <label className="text-xs font-semibold text-muted-foreground mb-1 block">{t('end_date')}</label>
                    ...
                    <div>
                      <label className="text-xs font-semibold text-muted-foreground mb-1 block">{t('payment_day')}</label>
                  ...
                  <div>
                    <label className="text-xs font-semibold text-muted-foreground mb-1 block">{t('extra_clauses')}</label>
                    <textarea value={state.extraClauses} onChange={e => set({ extraClauses: e.target.value })} rows={3}
                      placeholder={t('clauses_placeholder')} className="w-full px-3 py-2.5 rounded-xl border border-border bg-card text-sm outline-none focus:border-primary resize-none" />
                  </div>
                </>
              )}
            </div>
          ...
          {state.step === 4 && (
            <div className="space-y-4">
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-xs text-amber-700">
                <p className="font-bold flex items-center gap-1.5"><AlertTriangle className="w-4 h-4" /> {t('review_warning')}</p>
                <p className="mt-1">{t('review_warning_desc')}</p>
              </div>
            ...
            <div className="space-y-4">
              <div className="bg-card border border-border rounded-2xl p-6 shadow-card">
                <h3 className="font-bold text-foreground mb-2 flex items-center gap-2">
                  <PenSquare className="w-5 h-5 text-primary" /> {t('landlord_signature')}
                </h3>
                <p className="text-xs text-muted-foreground mb-4">{t('sign_instructions')}</p>
                ...
                <div className="flex items-center gap-2 mt-3">
                  ...
                  {hasSignature && (
                    <span className="flex items-center gap-1 text-xs font-bold text-success">
                      <CheckCircle2 className="w-3.5 h-3.5" /> {t('signature_captured')}
                    </span>
                  )}
                </div>
              </div>
          ...
          <div className="flex items-center justify-between mt-8 pt-6 border-t border-border">
            <button onClick={() => state.step > 1 ? set({ step: state.step - 1 }) : router.push('/dashboard/landlord')}
              ...
              <ChevronLeft className="w-3.5 h-3.5" /> {state.step > 1 ? t('previous') : t('cancel')}
            </button>
            ...
            {state.step < 5 ? (
              <button onClick={handleNext} disabled={!canGoNext()}
                ...
                {state.step === 3 ? t('preview') : t('next')} <ChevronRight className="w-3.5 h-3.5" />
              </button>
            ) : (
              <button onClick={handleSubmit} disabled={state.isSubmitting || !hasSignature}
                ...
                {state.isSubmitting ? (
                  ...
                  <><Send className="w-4 h-4" /> {t('sign_and_send')}</>
                )}
              </button>
            )}
          </div>
