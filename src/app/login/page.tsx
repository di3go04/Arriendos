...
function LoginPageContent() {
  ...
  // Helper to read search params without useSearchParams() (which causes
  // infinite Suspense in Next.js 16 client components during SSR).
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Demo mode shortcut: when DEMO_MODE is active, allow login with
    // demo@rentnow.app / Demo123! directly via NextAuth (bypassing Supabase).
    // This makes the demo usable for buyers evaluating the product.
    const isDemoMode = process.env.NEXT_PUBLIC_DEMO_MODE === 'true';
    const isDemoAccount = email.trim().toLowerCase() === 'demo@rentnow.app';

    if (isDemoMode && isDemoAccount) {
      if (password !== 'Demo123!') {
        setErrorMsg(t('auth.invalid_credentials'));
        setIsSubmitting(true);
        // Brief loading state to mimic real auth flow
      ...
      setIsSubmitting(true);
      try {
        // Use NextAuth credentials provider (configured for DEMO_MODE in src/lib/auth.ts)
        ...
        console.debug('Demo login error:', err);
    ...
    // Regular Supabase auth flow (for non-demo accounts / production)
    setErrorMsg('');
    setIsSubmitting(true);
    try {
      ...
      if (error) {
        ...
        if (message === 'Invalid login credentials') {
          ...
          // Default to "invalid credentials" for security (don't leak which users exist)
          setErrorMsg(t('auth.invalid_credentials'));
        }
        return;
      }
    ...
    <main className="min-h-screen flex bg-muted">
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-sm space-y-8">
          ...
          {/* Hint para compradores: credenciales demo visibles */}
          <div className="text-center text-xs text-muted-foreground/70 bg-muted/50 rounded-lg p-3 border border-border/50">
