...
function DemoContent() {
  ...
  const handleSimulatedSSO = useCallback(async (provider: string) => {
    ...
    authenticateSimulation(provider);
      toast({
        type: "success",
        message: t('sso_success', { provider }),
      });
          ...
          <div className="flex items-center gap-3">
            ...
            {isDemoAuthenticated && demoUser ? (
              <div className="flex items-center gap-3">
                ...
                <button
                  type="button"
                  onClick={() => { logoutSimulation(); toast({ type: "info", message: t('sso_logout') }); }}
              ...
              >
                {t('btn_cta')} <Zap className="w-4 h-4" />
              </Link>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
              {[
                { icon: Activity, label: t('events_label'), value: counters.pageEvents.toLocaleString(), color: "text-[#1e3a5f]" },
                { icon: Users, label: t('users_label'), value: counters.activeUsers.toLocaleString(), color: "text-[#f59e0b]" },
                { icon: BarChart3, label: t('conversion_label'), value: `${counters.conversionRate.toFixed(1)}%`, color: "text-emerald-600" },
                { icon: CreditCard, label: t('revenue_label'), value: `$${periodData.revenue.toLocaleString()}`, color: "text-[#1e3a5f]" },
              ].map((card) => (
            ...
            <div className="bg-white dark:bg-[#1E293B] rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm">
              <div className="flex items-center gap-2 px-5 py-3 border-b border-gray-100 dark:border-gray-700">
                ...
                <h3 className="text-xs font-bold text-[#64748B] dark:text-[#94A3B8] uppercase tracking-wider">{t('live_activity')}</h3>
                <span className="ml-auto text-[10px] text-[#94A3B8] font-mono">{t('events_count', { count: events.length })}</span>
              </div>
            ...
            <div className="bg-white dark:bg-[#1E293B] rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm">
              <div className="flex items-center gap-2 px-5 py-3 border-b border-gray-100 dark:border-gray-700">
                ...
                <h3 className="text-xs font-bold text-[#64748B] dark:text-[#94A3B8] uppercase tracking-wider">{t('session_replays')}</h3>
                <span className="ml-auto text-[10px] text-[#94A3B8] font-mono">{t('replays_count', { count: SESSION_REPLAYS.length })}</span>
              </div>
                ...
                {SESSION_REPLAYS.map((session, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center gap-4 px-5 py-3 hover:bg-gray-50 dark:hover:bg-[#0F172A]/50 transition-colors group">
                      <button
                        type="button"
                        onClick={() => toast({ type: "info", message: t('session_replays') + ': ' + session.label })}
          ...
          <div className="space-y-6">
            ...
            {!isDemoAuthenticated && (
              <div className="bg-white dark:bg-[#1E293B] rounded-2xl border border-gray-200 dark:border-gray-700 p-5 shadow-sm">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="text-xs font-bold text-[#64748B] dark:text-[#94A3B8] uppercase tracking-wider">{t('sso_title')}</h3>
                </div>
                ...
                <div className="space-y-3">
                  {[
                    { provider: "google", label: t('sso_google') },
                    { provider: "azure-ad", label: t('sso_azure') },
                  ].map(({ provider, label }) => (
                    <button
                      key={provider}
                      type="button"
                    ...
                    >
                      ...
                      {ssoLoading === provider ? t('sso_verifying') : label}
                    </button>
                  ))}
                </div>
              </div>
            ...
            {isDemoAuthenticated && demoUser && (
              <div className="bg-white dark:bg-[#1E293B] rounded-2xl border border-gray-200 dark:border-gray-700 p-5 shadow-sm">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="text-xs font-bold text-[#64748B] dark:text-[#94A3B8] uppercase tracking-wider">{t('sso_active')}</h3>
                </div>
                <div className="flex items-center gap-3 p-3 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-[#64748B] dark:text-[#94A3B8]">{t('sso_logged_as', { provider: demoUser.provider })}</p>
                  </div>
                </div>
            ...
            <div className="bg-white dark:bg-[#1E293B] rounded-2xl border border-gray-200 dark:border-gray-700 p-5 shadow-sm">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-xs font-bold text-[#64748B] dark:text-[#94A3B8] uppercase tracking-wider">{t('export_title')}</h3>
              </div>
              <p className="text-[10px] text-[#94A3B8] mb-4 leading-relaxed">
                {t('export_desc')}
              </p>
            ...
            <div className="bg-white dark:bg-[#1E293B] rounded-2xl border border-gray-200 dark:border-gray-700 p-5 shadow-sm">
              <div className="flex items-center gap-2 mb-3">
                <Shield className="w-4 h-4 text-[#1e3a5f]" />
                <h3 className="text-xs font-bold text-[#64748B] dark:text-[#94A3B8] uppercase tracking-wider">{t('gdpr_title')}</h3>
              </div>
              <ConsentBanner />
            </div>
          </div>
