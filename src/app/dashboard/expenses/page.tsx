...
export default function ExpensesPage() {
  const t = useTranslations('EXPENSES_PAGE');
  const { user } = useAuth();
  ...
  const fetchExpenses = async () => {
    try {
      ...
      if (!res.ok) throw new Error(t('error_load_expenses'));
      const data = await res.json();
      setExpenses(data.expenses || []);
      setSummary(data.summary);
    } catch {
      setErrorMsg(t('error_load_expenses_msg'));
    }
  };
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      try {
        ...
        setErrorMsg(t('error_load_properties'));
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(t('error_save_expense'));
      setShowForm(false);
      setForm({ property_id: '', category: 'maintenance', amount: '', description: '', expense_date: '' });
      await fetchExpenses();
    } catch {
      setErrorMsg(t('error_save_expense_msg'));
    }
  };
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      if (!res.ok) throw new Error(t('error_delete_expense'));
      await fetchExpenses();
    } catch {
      setErrorMsg(t('error_delete_expense_msg'));
    }
  };
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-card border border-border rounded-xl p-6 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-semibold text-muted-foreground block mb-1">{t('form_property')}</label>
              <select
              ...
              >
                <option value="">{t('form_select_property')}</option>
                {properties.map((p) => (
            ...
            <div>
              <label className="text-xs font-semibold text-muted-foreground block mb-1">{t('form_category')}</label>
              <select
                value={form.category}
            ...
            <div>
              <label className="text-xs font-semibold text-muted-foreground block mb-1">{t('form_amount')}</label>
              <input
                type="number"
                step="0.01"
                value={form.amount}
            ...
            <div>
              <label className="text-xs font-semibold text-muted-foreground block mb-1">{t('form_date')}</label>
              <input
                type="date"
          ...
          <div>
            <label className="text-xs font-semibold text-muted-foreground block mb-1">{t('form_description')}</label>
            <textarea
      ...
      {expenses.length === 0 ? (
        <div className="py-16 text-center space-y-3">
          ...
          <p className="font-semibold text-foreground">{t('empty_state_title')}</p>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-border bg-muted">
                  <th className="p-4 text-[10px] font-bold text-muted-foreground uppercase">{t('table_header_property')}</th>
                  <th className="p-4 text-[10px] font-bold text-muted-foreground uppercase">{t('table_header_category')}</th>
                  <th className="p-4 text-[10px] font-bold text-muted-foreground uppercase">{t('table_header_description')}</th>
                  <th className="p-4 text-[10px] font-bold text-muted-foreground uppercase">{t('table_header_date')}</th>
                  <th className="p-4 text-[10px] font-bold text-muted-foreground uppercase">{t('table_header_amount')}</th>
