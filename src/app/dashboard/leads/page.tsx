...
export default function LeadsPage() {
  const t = useTranslations('LEADS_PAGE');
  const { user } = useAuth();
  ...
  async function loadLeads() {
    ...
    if (data) {
      setLeads((data as (Lead & { properties?: { title?: string } })[]).map((l) => ({
        ...l,
        property_title: l.properties?.title || t('no_property'),
      })));
    }
    setLoading(false);
  }
    ...
    <main className="min-h-screen bg-background p-4 md:p-6">
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-xl font-black text-foreground">{t('page_title')}</h1>
            <p className="text-xs text-muted-foreground mt-1">
              {t('leads_received', { count: leads.length })}
            </p>
          </div>
          <button onClick={loadLeads} className="inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-border text-xs font-semibold text-foreground hover:bg-muted transition-colors cursor-pointer">
            <RefreshCw className="w-3.5 h-3.5" /> {t('refresh_button')}
          </button>
        </div>
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-sm font-semibold text-muted-foreground">{t('empty_state_title')}</p>
            <p className="text-xs text-muted-foreground/60 mt-1">{t('empty_state_description')}</p>
          </div>
        ) : (
