...
function StatusBadge({ status }: { status: string }) {
  ...
  const labels: Record<string, string> = {
    activo: 'Activo', firmado: 'Firmado', borrador: 'Borrador',
    pendiente_firma: 'Pendiente', finalizado: 'Finalizado', cancelado: 'Cancelado',
  };
  return (
  ...
  useEffect(() => {
    ...
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        console.error(err);
        toast({ type: 'error', message: 'Error al cargar el panel.' });
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center gap-3">
        ...
        <p className="text-xs font-semibold text-muted-foreground">Cargando panel...</p>
      </div>
    );
  }
    ...
    <div className="space-y-8 pb-12 animate-fade-in">
      ...
      <div className="relative overflow-hidden rounded-[20px] p-6 md:p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-card-hover">
        ...
        <div className="relative z-10">
          ...
          <p className="text-xs text-blue-100/80 mt-1.5 font-medium max-w-xl">
            Panel de control de tus propiedades, contratos e ingresos.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <div className="bg-card border border-border rounded-[16px] p-5 shadow-card hover:shadow-card-hover hover:-translate-y-0.5 transition-all duration-300">
          ...
          <span className="block text-[11px] font-bold text-ink-secondary uppercase tracking-wider">Propiedades</span>
          <span className="block text-4xl font-bold tracking-tight text-foreground mt-1 tabular-nums">{stats.totalProperties}</span>
          <Link href="/dashboard/properties" className="block text-[10px] text-primary hover:text-primary-hover font-medium mt-2 transition-colors">Ver catálogo →</Link>
        </div>
        ...
        <div className="bg-card border border-border rounded-[16px] p-5 shadow-card hover:shadow-card-hover hover:-translate-y-0.5 transition-all duration-300">
          ...
          <span className="block text-[11px] font-bold text-ink-secondary uppercase tracking-wider">Contratos</span>
          ...
          <span className="block text-[10px] text-ink-secondary mt-2 font-bold flex items-center gap-1">
            <CheckCircle className="w-3 h-3 text-success" /> Estable
          </span>
        </div>
        ...
        <div className="bg-card border border-border rounded-[16px] p-5 shadow-card hover:shadow-card-hover hover:-translate-y-0.5 transition-all duration-300">
          ...
          <span className="block text-[11px] font-bold text-ink-secondary uppercase tracking-wider">Renta Esperada</span>
          ...
          <span className="block text-[10px] text-muted-foreground mt-2 font-semibold">Mes actual</span>
        </div>
        ...
        <div className="bg-card border border-border rounded-[16px] p-5 shadow-card hover:shadow-card-hover hover:-translate-y-0.5 transition-all duration-300">
          ...
          <span className="block text-[11px] font-bold text-ink-secondary uppercase tracking-wider">Vencimientos</span>
          <span className="block text-4xl font-bold tracking-tight text-foreground mt-1 tabular-nums">{stats.upcomingThisWeek}</span>
          <Link href="/dashboard/payments" className="block text-[10px] text-primary hover:text-primary-hover font-medium mt-2 transition-colors">Ir a conciliación →</Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-card border border-border rounded-[16px] p-6 shadow-card lg:col-span-2">
          <div className="flex items-center justify-between border-b border-border-subtle pb-4 mb-6">
            <div>
              <h3 className="text-base font-bold text-foreground" style={{ fontFamily: 'Poppins, sans-serif' }}>Flujo de Renta</h3>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="px-3 py-1.5 rounded-lg bg-primary-subtle border border-primary/20 text-primary flex items-center gap-1.5 text-[11px] font-bold">
                <TrendingUp className="w-3.5 h-3.5" /> Cobrado
              </span>
            </div>
          </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                ...
                <Tooltip cursor={{ fill: 'rgba(37,99,235,0.04)' }}
                  ...
                  formatter={(value: LooseValue) => [formatCOP(Number(value)), 'Cobrado']} />
        ...
        <div className="bg-card border border-border rounded-[16px] p-6 shadow-card flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-2 border-b border-border-subtle pb-3.5 mb-5">
              ...
              <h3 className="text-sm font-bold text-foreground" style={{ fontFamily: 'Poppins, sans-serif' }}>Resumen Operativo</h3>
            </div>
            ...
            <div className="space-y-3">
              {[{ label: 'Disponibles', value: properties.filter(p => p.status === 'disponible').length, cls: 'text-foreground' },
                { label: 'Borradores', value: contracts.filter(c => c.status === 'borrador').length, cls: 'text-warning' },
                { label: 'Total renta', value: formatCOP(contracts.filter(c => c.status === 'activo' || c.status === 'firmado').reduce((s, c) => s + Number(c.monthly_rent || 0), 0)), cls: 'text-foreground' },
              ].map((item, i) => (
          ...
          <div className="mt-5 pt-3.5 border-t border-border-subtle text-[10px] text-muted-foreground font-medium text-center">Datos actualizados en vivo</div>
        </div>
      </div>
        ...
        <div className="bg-card border border-border rounded-[16px] p-6 shadow-card">
          <div className="flex items-center justify-between border-b border-border-subtle pb-4 mb-5">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-foreground" style={{ fontFamily: 'Poppins, sans-serif' }}>Leads Recientes</h3>
            </div>
            <Link href="/propiedades" className="text-xs font-bold text-primary hover:text-primary-hover transition-colors flex items-center gap-1">
              Ver portal <ArrowUpRight className="w-3.5 h-3.5" />
            </Link>
          </div>
          ...
          {leads.length === 0 ? (
            <div className="py-10 text-center space-y-3">
              ...
              <p className="text-xs font-semibold text-foreground">Sin leads aún</p>
              <p className="text-[10px] text-muted-foreground max-w-[240px] mx-auto">Los interesados en tus propiedades aparecerán aquí cuando alguien solicite información desde el portal público.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {leads.map((lead, i) => (
        ...
        <div className="bg-card border border-border rounded-[16px] p-6 shadow-card">
          <div className="flex items-center justify-between border-b border-border-subtle pb-4 mb-5">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-foreground" style={{ fontFamily: 'Poppins, sans-serif' }}>Predicción de Riesgo IA</h3>
            </div>
          ...
          {riskScores.length === 0 ? (
            <div className="py-10 text-center space-y-3">
              ...
              <p className="text-xs font-semibold text-foreground">Sin datos de inquilinos</p>
              <p className="text-[10px] text-muted-foreground max-w-[240px] mx-auto">Necesitas inquilinos con historial de pagos para generar predicciones de riesgo.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {riskScores.map((r: LooseValue, i: number) => {
                ...
                const riskLabel = r.risk === 'low' ? 'Bajo' : r.risk === 'medium' ? 'Medio' : r.risk === 'high' ? 'Alto' : 'Crítico';
                return (
                  <div key={r.tenantId || i} className="p-4 rounded-xl bg-muted/30 border border-border/30 hover:bg-muted/50 transition-colors">
                    ...
                    <div className="flex items-center justify-between text-[10px] text-muted-foreground">
                      <span>Pagos: {r.paymentCount} ({r.unpaidCount} impagos)</span>
      ...
      <div className="bg-card border border-border rounded-[16px] shadow-card overflow-hidden">
        <div className="flex items-center justify-between border-b border-border-subtle p-6">
          <div>
            <h3 className="text-base font-bold text-foreground" style={{ fontFamily: 'Poppins, sans-serif' }}>Últimos Contratos</h3>
            <p className="text-[11px] text-muted-foreground mt-0.5 font-medium">Los más recientes</p>
          </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-border-subtle bg-muted">
                {['Contrato #', 'Propiedad', 'Arrendatario', 'Estado', 'Renta', 'Inicio'].map(h => (
              ...
              {recentContracts.length === 0 ? (
                ...
                recentContracts.map((c) => (
                  <tr key={c.id} className="hover:bg-muted transition-colors text-xs">
                    ...
                    <td className="p-4 font-semibold text-ink-secondary">{c.tenant?.full_name || 'Sin Asignar'}</td>
        ...
        <div className="md:hidden divide-y divide-border-subtle">
          {recentContracts.length === 0 ? (
            <div className="p-8 text-center text-xs text-muted-foreground font-medium">No tienes ningún contrato emitido todavía.</div>
          ) : (
            recentContracts.map((c) => (
              <div key={c.id} className="p-4 space-y-2.5 hover:bg-muted transition-colors">
                ...
                <div className="grid grid-cols-2 gap-2 text-[11px]">
                  <div><span className="text-muted-foreground font-medium">Propiedad: </span><span className="text-foreground font-semibold">{c.property?.title || 'Inmueble'}</span></div>
                  <div><span className="text-muted-foreground font-medium">Inquilino: </span><span className="text-foreground font-semibold">{c.tenant?.full_name || 'Sin Asignar'}</span></div>
                  <div><span className="text-muted-foreground font-medium">Renta: </span><span className="text-foreground font-bold">{formatCOP(c.monthly_rent)}</span></div>
                  <div><span className="text-muted-foreground font-medium">Inicio: </span><span className="text-foreground">{new Date(c.start_date).toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit', year: 'numeric' })}</span></div>
                </div>
              </div>
            ))
          )}
        ...
        steps={[
          {
            ...
            content: 'Aquí puedes ver un resumen completo de tus propiedades, contratos, ingresos y leads en tiempo real.',
          },
          {
            target: '',
            title: 'Leads Recientes',
            content: 'Los interesados en tus propiedades desde el portal público aparecen aquí. Contáctalos pronto para cerrar el alquiler.',
          },
          {
            target: '',
            title: 'Predicción de Riesgo IA',
            content: 'Nuestra IA analiza el historial de pagos de cada inquilino y predice el riesgo de morosidad. Los scores altos requieren atención.',
          },
          {
            target: '',
            title: 'Exporta tus Reportes',
            content: 'Puedes exportar todos tus datos a Excel para llevar tu contabilidad. Usa el botón "Exportar a Excel" en la sección de pagos.',
          },
          {
            target: '',
            title: '¡Todo listo!',
            content: 'Ya conoces las funciones principales. Explora el menú lateral para descubrir más herramientas. ¡Éxito gestionando tus arriendos!',
          },
        ]}
