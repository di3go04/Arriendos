...
export default function UsarPlantillaPage({
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (!resTmpl.ok) throw new Error('Error al cargar la plantilla');
        if (!resArrs.ok) throw new Error('Error al cargar los arriendos');
  ...
  const handleGenerar = () => {
    ...
    if (!arriendo) {
      setError('Arriendo no encontrado');
      setGenerando(false);
      return;
    }
    ...
    try {
      ...
      setError((err as { message?: string }).message || 'Error al generar el documento');
    } finally {
      setGenerando(false);
    }
  };
  ...
  const handleDescargarPDF = async () => {
    ...
    try {
      ...
      setError('Error al generar PDF: ' + ((err as { message?: string }).message || 'desconocido'));
    }
  };
            ...
            <div className="bg-card border border-border rounded-2xl shadow-card p-5 space-y-4">
                ...
                <div className="space-y-2.5 pt-1 animate-fade-in">
                  <div className="flex items-start gap-2.5 text-xs">
                    ...
                    <span className="text-foreground font-semibold">
                      {selectedArriendo.property?.title || 'Sin propiedad'}
                    </span>
                  </div>
                  <div className="flex items-start gap-2.5 text-xs">
                    ...
                    <span className="text-foreground font-semibold">
                      ...
                      <span className="text-muted-foreground font-normal"> (Inquilino)</span>
                    </span>
                  </div>
                  <div className="flex items-start gap-2.5 text-xs">
                  ...
                  <div className="flex items-start gap-2.5 text-xs">
                    ...
                    <span className="text-foreground font-semibold">
                      {formatCOP(selectedArriendo.monthly_rent)}/mes
                    </span>
                  </div>
                </div>
              )}
              ...
              >
                {generando ? (
                  ...
                  <><Eye className="w-4 h-4" /> Generar documento</>
                )}
              </button>
            </div>
            ...
            {template && (
              <div className="bg-primary/5 border border-primary/10 p-4 rounded-2xl space-y-1.5">
                ...
                <p className="text-[11px] text-muted-foreground leading-relaxed">
                  Tipo: <strong>{template.tipo === 'ia' ? 'Generada por IA' : 'Manual'}</strong>
                  {' \u2022 '}
                  Variables: <strong>{template.variables?.length || 0}</strong>
                </p>
              </div>
            )}
          </div>
              ...
              <div className="px-5 py-3.5 border-b border-border flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold text-foreground">
                    {documentoHtml ? 'Documento generado' : 'Vista previa'}
                  </span>
                </div>
                {documentoHtml && (
                  <button
                ...
                {!documentoHtml ? (
                  <div className="flex flex-col items-center justify-center py-20 text-center space-y-3">
                    ...
                    <p className="text-xs font-semibold text-muted-foreground">
                      Selecciona un arriendo y haz clic en <strong>&quot;Generar documento&quot;</strong>
                    </p>
                    <p className="text-[10px] text-muted-foreground/60">
                      El HTML de la plantilla se combinará con los datos reales del arriendo.
                    </p>
                  </div>
                ) : (
