...
export default function CrearPlantillaPage() {
  ...
  const handleGenerarIA = async () => {
    if (!descripcion.trim()) {
      setError('Escribe una descripcion para generar la plantilla');
      return;
    }
    ...
    try {
      ...
      if (!titulo.trim()) {
        setTitulo('Plantilla generada por IA');
      }
    } catch (err: unknown) {
      setError((err as { message?: string }).message || 'Error al generar la plantilla');
    } finally {
      setGenerando(false);
    }
  };
  ...
  const handleGuardar = async () => {
    ...
    if (!htmlAGuardar.trim() || htmlAGuardar.trim() === '<p><br></p>') {
      setError('El contenido de la plantilla no puede estar vacio');
      return;
    }
      ...
      <div className="mb-2">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground font-semibold mt-0.5">
              {modo === 'manual'
                ? 'Disena una plantilla visualmente o inserta campos dinamicos'
                : 'Genera una plantilla describiendo lo que necesitas en lenguaje natural'}
            </p>
          </div>
        </div>
        ...
        <div className="bg-card border border-border rounded-2xl shadow-card overflow-hidden">
          ...
          <div className="p-4 md:p-6">
            {/* Modo Manual: HTML editor */}
            <div className={modo === 'manual' ? '' : 'hidden'}>
              <textarea
                ref={editorRef}
                value={contenido}
                onChange={(e) => setContenido(e.target.value)}
                placeholder="Escribe el contenido de la plantilla aqui. Usa el boton 'Insertar campo' para agregar campos dinamicos como {{nombre_arrendatario}}."
                rows={18}
              ...
              <div className="space-y-5 animate-fade-in">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2 pt-1">
                    <span className="text-[10px] font-semibold text-muted-foreground leading-7">Sugerencias:</span>
                ...
                {generando && (
                  <div className="space-y-3 border border-border p-6 rounded-xl bg-muted/20 animate-pulse">
                    ...
                    <p className="text-[10px] text-center text-primary font-bold pt-2">
                      Gemini esta redactando tu plantilla...
                    </p>
                  </div>
                )}
                  ...
                  <div ref={previewRef} className="space-y-3 border border-primary/20 rounded-xl overflow-hidden animate-fade-in">
                    <div className="flex items-center justify-between px-4 py-2.5 bg-primary/5 border-b border-primary/10">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold text-primary">Vista previa de la plantilla generada</span>
                      </div>
                      <span className="text-[10px] text-muted-foreground bg-card px-2 py-1 rounded-md border border-border">
                        Generado por Gemini - Puedes editarlo en Modo Manual
                      </span>
                    </div>
        ...
        <div className="bg-primary/5 border border-primary/10 p-4 rounded-2xl flex items-start gap-3">
          ...
          {modo === 'manual' ? (
            <div>
              <p className="text-xs font-bold text-primary mb-0.5">Como funciona?</p>
              <p className="text-[11px] text-muted-foreground leading-relaxed">
                Usa el boton <strong>&quot;Insertar campo&quot;</strong> para agregar campos dinamicos en el texto.
            ...
            <div>
              <p className="text-xs font-bold text-primary mb-0.5">Como funciona el Modo IA?</p>
              <p className="text-[11px] text-muted-foreground leading-relaxed">
