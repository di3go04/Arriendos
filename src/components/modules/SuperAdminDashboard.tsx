    ...
    <div className="min-h-screen bg-neutral-100 dark:bg-neutral-950 p-8">
      <div className="max-w-7xl mx-auto space-y-8">
        <header className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold">Super Admin — Multi-tenant</h1>
            <p className="text-neutral-500 mt-1">Módulo superadmin-tenant</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <MetricCard icon={Building} label="Propiedades" value={metrics?.properties ?? 0} />
          <MetricCard icon={Users} label="Contratos" value={metrics?.contracts ?? 0} />
          <MetricCard icon={Activity} label="Leads" value={metrics?.leads ?? 0} />
        </div>

        <section className="bg-white dark:bg-neutral-900 rounded-2xl border p-6">
          <h2 className="font-bold mb-4 flex items-center gap-2">
            <DollarSign className="w-5 h-5" /> Organizaciones
          </h2>
          {orgs.length === 0 ? (
            <p className="text-sm text-neutral-500">Sin organizaciones o sin service role.</p>
          ) : (
