...
import { useNotifications, getTypeIcon, getTypeLabel, getNotificationLink, type AppNotification } from '@/hooks/useNotifications';
            ...
            <div className="absolute right-0 mt-2 w-[340px] md:w-[380px] bg-card border border-border rounded-xl shadow-modal z-50 animate-fade-in origin-top-right flex flex-col max-h-[520px]">
              <div className="flex items-center justify-between px-4 py-3 border-b border-border shrink-0">
                      <span className="font-semibold text-sm text-foreground flex items-center gap-2">
                        ...
                        {t('title')}
                        {unreadCount > 0 && (
                          <span className="bg-primary/10 text-primary text-[9px] font-bold px-2 py-0.5 rounded-full">{t('new_badge', { count: unreadCount })}</span>
                        )}
                      </span>
                ...
                {typeFilters.slice(0, 6).map(type => (
                  <button key={type} onClick={() => setFilter(type)}
                    ...
                    {type === 'all' ? t('all_filter') : type === 'unread' ? t('unread_filter') : getTypeLabel(type)}
                  </button>
                ))}
              </div>
              ...
              <div className="flex-1 overflow-y-auto">
                {loading && notifications.length === 0 ? (
                  ...
                  <div className="py-12 text-center space-y-2">
                    ...
                    <p className="text-[10px] text-ink-muted max-w-[200px] mx-auto">{filter !== 'all' ? t('no_filter_match') : t('no_notifications_desc')}</p>
                  </div>
                ) : (
                      ...
                      return (
                        <div key={n.id} onClick={() => handleNotificationClick(n)}
                          ...
                          <div className="flex items-center gap-2 mt-1.5 ml-8 opacity-0 group-hover:opacity-100 transition-opacity">
                            {!n.read && <button onClick={(e) => { e.stopPropagation(); markAsRead(n.id); }} className="text-[8px] font-semibold text-primary hover:underline flex items-center gap-0.5 cursor-pointer"><EyeOff className="w-2.5 h-2.5" /> {t('mark_read')}</button>}
                          </div>
                        </div>
                      );
                    })}
              ...
              {notifications.length > 0 && (
                <div className="flex items-center justify-between px-4 py-2 border-t border-border shrink-0 bg-muted/30 rounded-b-xl">
                  <button onClick={markAllAsRead} disabled={unreadCount === 0}
                    ...
                    <Check className="w-3 h-3" /> {t('mark_all_read_short')}
                  </button>
                  <span className="text-[8px] text-ink-muted">{t('total_count', { count: notifications.length })}</span>
                </div>
              )}
            </div>
          )}
