...
const menuItemDefs = [
  { key: 'home', icon: Home, href: (role: string) => role ? '/dashboard' : '/', roles: ['arrendador', 'arrendatario', 'admin'] },
  { key: 'dashboard', icon: LayoutDashboard, href: (role: string) => role === 'arrendador' ? '/app/landlord' : '/app/tenant' },
  { key: 'properties', icon: Building2, href: () => '/properties', roles: ['arrendador'] },
  { key: 'templates', icon: FileCode, href: () => '/templates', roles: ['arrendador'] },
  { key: 'contracts', icon: FileText, href: () => '/app/leases' },
  { key: 'payments', icon: DollarSign, href: () => '/app/payments' },
  { key: 'expenses', icon: DollarSign, href: () => '/app/expenses', roles: ['arrendador'] },
  { key: 'reports', icon: FileSpreadsheet, href: () => '/app/reports', roles: ['arrendador'] },
  { key: 'incidents', icon: Wrench, href: () => '/app/maintenance' },
  { key: 'documents', icon: FileCode, href: () => '/app/tenant/documents', roles: ['arrendatario'] },
  { key: 'pricing', icon: Tag, href: () => '/precios' },
  { key: 'status', icon: ShieldCheck, href: () => '/status' },
  { key: 'settings', icon: Settings, href: () => '/app/settings' },
];

function ThemeToggleButton({ collapsed, t }: { collapsed: boolean; t: (key: string) => string }) {
  ...
  return (
    ...
    >
      ...
      {!collapsed && <span>{theme === 'dark' ? t('sidebar.theme_light') : t('sidebar.theme_dark')}</span>}
    </button>
  );
}
...
export default function Sidebar({
  ...
  const menuItems = menuItemDefs.map(item => ({ ...item, label: t(`sidebar.${item.key}`) }));
    ...
    <>
      ...
      >
        <div className="px-5 py-6 border-b border-border flex items-center justify-between">
          <Logo
            onClick={onCloseMobile}
            className={collapsed ? 'justify-center w-full' : 'gap-2'}
        ...
        <div className="p-3 border-t border-border">
          <div className={`flex items-center gap-2.5 px-2 py-2 ${collapsed ? 'justify-center' : ''}`}>
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-brand to-primary flex items-center justify-center text-white text-xs font-bold shrink-0">
              {profile?.avatar_url ? (
                <img src={profile.avatar_url} alt={profile.full_name || t('sidebar.user_fallback')} className="w-full h-full object-cover rounded-full" />
              ) : (
            ...
            {!collapsed && (
              <div className="min-w-0 flex-1">
                <p className="text-xs font-semibold text-foreground truncate">{profile?.full_name || t('sidebar.user_fallback')}</p>
                <p className="text-[9px] text-muted-foreground uppercase tracking-wider font-medium">
                  {profile?.role === 'arrendador' ? t('sidebar.landlord_role') : t('sidebar.tenant_role')}
                </p>
              </div>
            )}
          </div>
          ...
          <div className={`flex gap-2 mt-2 ${collapsed ? 'flex-col' : ''}`}>
            <ThemeToggleButton collapsed={collapsed} t={t} />
            {/* Language switcher */}
            <AppLanguageSwitcher collapsed={collapsed} />
            <button
              onClick={() => {
