...
export function MorosityWidget({ data }: { data: MorosityData }) {
  const t = useTranslations('morosity')
    ...
    <div className={`rounded-2xl p-5 border ${colors.bg} ${colors.border}`}>
      <div className="flex items-start justify-between mb-4">
        ...
        <div className={`flex items-center gap-1 text-xs font-bold ${colors.text}`}>
          ...
          {data.trend === 'improving' ? t('trend_improving') : data.trend === 'worsening' ? t('trend_worsening') : t('trend_stable')}
        </div>
      </div>

      <div className="space-y-3">
        <div className="flex justify-between text-xs">
          <span className="text-muted-foreground">{t('overdue_count', { count: data.overdueCount, total: data.totalContracts })}</span>
          <span className="font-bold text-foreground">{data.overdueCount > 0 ? t('alert') : t('no_issues')}</span>
        </div>
        ...
        <p className="text-[10px] text-muted-foreground">
          {data.riskLevel === 'low' && t('risk_low')}
          {data.riskLevel === 'medium' && t('risk_medium')}
          {data.riskLevel === 'high' && t('risk_high')}
        </p>
      </div>
    </div>
  );
