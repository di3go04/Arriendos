...
Las siguientes integraciones son opcionales. El producto funciona sin ellas, pero se activan al configurar sus variables de entorno:

| API | Propósito | Variable de entorno |
|---|---|---|
| Stripe | Pagos internacionales USD | `STRIPE_SECRET_KEY` |
| Mercado Pago | Pagos LATAM | `MP_ACCESS_TOKEN` |
| PayPal | Suscripciones | `PAYPAL_CLIENT_ID` + `PAYPAL_SECRET` |
| Google Gemini | Generación de contratos con IA | `GEMINI_API_KEY` |
| Belvo | Open Banking (KYC) | `BELVO_SECRET_ID` + `BELVO_SECRET_PASSWORD` |
| Vapi.ai | Agente de voz IA | `VAPI_API_KEY` |
| Twilio | SMS y voz | `TWILIO_ACCOUNT_SID` + `TWILIO_AUTH_TOKEN` |
| Resend | Emails transaccionales | `RESEND_API_KEY` |
| WhatsApp Baileys | Recordatorios de pago | `WHATSAPP_BRIDGE_URL` |
| Mapbox | Mapa interactivo | `NEXT_PUBLIC_MAPBOX_TOKEN` (o usa OpenStreetMap gratis) |

> En modo demo (`DEMO_MODE=true`), el producto funciona sin ninguna de estas APIs.
...
- **Login demo:** https://arriendos-kappa.vercel.app/login-direct (botón ⚡ Acceder con cuenta demo)
- **Dashboard:** https://arriendos-kappa.vercel.app/app
- **Login demo:** `demo@rentnow.app` / `Demo123!` (o usa el botón ⚡ en /login-direct)
